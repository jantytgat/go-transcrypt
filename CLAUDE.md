...
The key floor (`minKeyLength`) applies on encrypt; decrypt only requires a non-empty key, matching the compat policy. Header tampering always fails (suite/salt bytes feed key derivation); a `Target` that is a symlink is replaced as a link, not followed. Files never enter the reflection converters, so the three type lists are unaffected.
